class EnsureSolidStackTables < ActiveRecord::Migration[8.1]
  def up
    # Solid Queue tables
    unless table_exists?(:solid_queue_jobs)
      ...
      add_index :solid_queue_jobs, [:queue_name, :finished_at], name: "index_sq_jobs_for_filtering"
      add_index :solid_queue_jobs, [:scheduled_at, :finished_at], name: "index_sq_jobs_for_alerting"
    end
    ...
    unless table_exists?(:solid_queue_blocked_executions)
      ...
      add_index :solid_queue_blocked_executions, [:concurrency_key, :priority, :job_id], name: "index_sq_blocked_for_release"
      add_index :solid_queue_blocked_executions, [:expires_at, :concurrency_key], name: "index_sq_blocked_for_maintenance"
    ...
    unless table_exists?(:solid_queue_ready_executions)
      ...
      add_index :solid_queue_ready_executions, [:priority, :job_id], name: "index_sq_poll_all"
      add_index :solid_queue_ready_executions, [:queue_name, :priority, :job_id], name: "index_sq_poll_by_queue"
    ...
    unless table_exists?(:solid_queue_scheduled_executions)
      ...
      add_index :solid_queue_scheduled_executions, [:scheduled_at, :priority, :job_id], name: "index_sq_dispatch_all"
  ...
  def down
    # Intentionally left blank - don't drop these tables
  end
end
