...
# 싱가포르 리전에 Redis 및 Solid Queue 설정 스크립트
echo "=== Render 싱가포르 리전 설정 스크립트 ==="
...
echo "1. singapore-redis: 싱가포르 리전의 Redis 인스턴스 (캐시용)"
echo "2. talkk-api: 싱가포르 리전의 API 서버"
echo "   - Solid Queue worker는 Procfile에서 함께 관리됩니다."
echo ""
echo "배포 후 확인할 사항:"
echo "- Render 대시보드에서 서비스 로그 확인"
echo "- API 서버가 Redis에 연결되는지 확인"
echo "- Solid Queue가 작업을 처리하는지 확인 (rails diagnostic:check_all)"
echo ""
echo "설정 스크립트 완료."
